...
public class HeapImpl<T extends Comparable<T>> implements Heap<T> {
	...
	private void heapify(int position) {
		if(position >= 0 && position < this.size()){
			...
			if(l <= this.index && comparator.compare(this.getHeap()[l], this.getHeap()[position]) > 0){
				largest = l;
			}
			if(r <= this.index && comparator.compare(this.getHeap()[r], this.getHeap()[largest]) > 0){
				largest = r;
			}
			if(largest != position){
	...
	public void insert(T element) {
		...
		if(element != null){
			int i = ++this.index;
			while(i > 0 && comparator.compare(this.getHeap()[parent(i)], element) < 0){
				this.getHeap()[i] = this.getHeap()[parent(i)];
				i = parent(i);
			}
			this.getHeap()[i] = element;
		}
	}
	...
	public void buildHeap(T[] array) {
		if(array != null){
			...
			for(int i = (this.index - 1)/2; i >= 0; i--){
				this.heapify(i);
			}
		}
	}
